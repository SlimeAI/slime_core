...
def _create_func(func: FuncOrMethod, cls: type, name: str, created: Union[list, tuple]):
    """
        ...
        *,
        ...
        def wrapper(func: Callable):
            if not is_none_or_nothing(self_func) and use_wraps:
                ...
                func = _create_func(
                    func=func, cls=self.cls, name=self.name, created=created
                )
...
@overload
def MetaclassCheck(
    _cls, *, ignored_metaclasses: Union[Tuple[Type[type], ...], Missing] = MISSING
) -> Type[_T]:
    pass
...
def MetaclassCheck(
    ...
    *,
    ...
    def decorator(cls: Type[_T]) -> Type[_T]:
        ignored = set(ignored_metaclasses if ignored_metaclasses is not MISSING else ())
        ...
        for base in bases:
            ...
            while len(base_metaclass_mro) > 0:
                ...
                base_metaclass_mro = list(
                    filter(
                        lambda _cls: _cls not in metaclass_mro_set, base_metaclass_mro
                    )
                )
        return cls
