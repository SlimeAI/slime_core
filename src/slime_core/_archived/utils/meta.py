...
class _MetaWrapper:
    ...
    def __init__(self, cls, *args: Any, **kwargs: Any) -> None:
        ...
        # set meta info
        args_str = _concat_format(
            "", [str(arg) for arg in args], "", item_sep=", ", break_line=False
        )
        kwargs_str = _concat_format(
            "", dict_to_key_value_str_list(kwargs), "", item_sep=", ", break_line=False
        )
        meta_str = _concat_format(
            "",
            [item for item in [args_str, kwargs_str] if len(item) > 0],
            "",
            item_sep=", ",
            break_line=False,
        )
...
@overload
def _Meta(
    _cls: Missing = MISSING, *, directly_new_allowed: bool = True
) -> Callable[[Type[_T]], Type[_T]]:
    pass


@overload
def _Meta(_cls: Type[_T], *, directly_new_allowed: bool = True) -> Type[_T]:
    pass


@DecoratorCall(index=0, keyword="_cls")
def _Meta(
    _cls: Union[Type[_T], Missing] = MISSING, *, directly_new_allowed: bool = True
):
    """
    ...
    def decorator(cls__: Type[_T]) -> Type[_T]:
        if not hasattr(cls__, "m_init__"):
            raise TypeError(
                f"Class ``{cls__.__name__}`` with ``Meta`` should have a ``m_init__`` method, but not found."
            )
