...
class Scoped(
    ...
    class_attr_compute__ = (
        ClassAttrCompute("escaped_scoped_attrs__", "escaped_scoped_attrs_computed__"),
    )
    ...
    def __setattr__(self, __name: str, __value: Any) -> None:
        if (
            not self.is_scoped_guard_enabled__()
            or __name in self.escaped_scoped_attrs_computed__
            or len(self.scoped_guards__) == 0
        ):
    ...
    def __delattr__(self, __name: str) -> None:
        ...
        # Pass the attribute to the guard container.
        return self.scoped_guards__.delattr_guard(self, super().__delattr__, __name)
...
_ATTR_OBSERVABLE_ESCAPED_SETATTRS = frozenset(
    ["is_scoped_guard_enabled__", "escaped_scoped_attrs_computed__", "scoped_guards__"]
)
