...
class BaseList(BaseListABC[_T], Generic[_T]):
    ...
    @InitOnce
    def __init__(self, __list_like: Union[Iterable[_T], EmptyFlag] = MISSING):
    ...
    def create__(
        ...
        *,
        ...
        ```
        """
        if return_constant and is_slime_constant(__list_like):
            return __list_like
...
class BiList(BaseList[_BiListItemT], BiListABC[_BiListItemT], Generic[_BiListItemT]):
    ...
    @overload
    def __setitem__(self, __key: SupportsIndex, __value: _BiListItemT) -> None:
        pass

    @overload
    def __setitem__(self, __key: slice, __value: Iterable[_BiListItemT]) -> None:
        pass
...
class BaseGenerator(
    ...
    def throw(self, __exc_type, __exc_value=None, __traceback=None) -> _YieldT_co:
        return self.call__(
            partial(self.gen.throw, __exc_type, __exc_value, __traceback)
        )
...
def CompositeDFT(
    __item: _CompositeStructureT, __func: Callable[[_CompositeStructureT], None]
) -> None:
    stack: Deque[Union[Iterator[_CompositeStructureT], Nothing]] = deque(
        [iter([__item])]
    )
...
def CompositeDFS(
    __item: _CompositeStructureT, __func: Callable[[_CompositeStructureT], bool]
...
def CompositeBFT(
    ...
    queue: Deque[Union[Iterator[_CompositeStructureT], Nothing]] = deque(
        [iter([__item])]
    )
...
class AttrObserver(AttrObserverABC):
    ...
    def observe_inspect__(
        ...
        for func_name in (
            ...
            if OBSERVE_FUNC_SUFFIX_PATTERN.search(func_name) is not None
        ):
...
class AttrObservable(AttrObservableABC[AttrObserver]):
    ...
    def detach__(
        ...
        *,
        ...
        detach_names = (
            set(__observer.detach_inspect__(namespaces).keys()) & detach_names
        )
