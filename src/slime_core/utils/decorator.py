...
from .typing.extension import MISSING, Missing, unwrap_method
    ...
    def decorator(func: _FuncOrMethodT) -> _FuncOrMethodT:
        ...
        def wrapper(self, *args, **kwargs) -> None:
            ...
            if getattr_func is MISSING:
                try:
                    init_once__: Dict = object.__getattribute__(
                        self, INIT_ONCE_ATTR_NAME
                    )
                except AttributeError:
                    init_once__ = MISSING
            else:
                init_once__: Union[Dict, Missing] = getattr_func(
                    self, INIT_ONCE_ATTR_NAME, MISSING
                )
